...
class TransformerDecoder(tf.keras.Model):
            ...
            self.feed_forward = [
                ...
                + [
                    tf.keras.layers.Dense(
                        ...
                        kernel_constraint=None,
                        bias_constraint=None,
                        name=f"fully-connected_{ff_idx}_layer_{depth - 1}",
                    )
                ]
    ...
    def call(self, inputs):
        ...
        for idx in range(self.decoding_reps):
            ...
            residual_stream = self.layer_norms[idx][1](ff_argument + residual_stream)
    ...
    def autoregressive_sampling(self, initial_states):
        ...
        for idx in range(self.sequence_length):
            conditional_logits, phases = self.call(inputs=autoregressive_spin_states)
    ...
    def evaluate_state(self, state, z_symmetric=False):
        ...
        log_probs = tf.reduce_sum(
            tf.gather(conditional_logits[:, :-1, :], state, batch_dims=2),
            axis=-1,
        )

        if z_symmetric:
            flog_probs, ftotal_phases = self.evaluate_state(
                1 - state, z_symmetric=False
            )
    ...
    def evaluate_gradients(
        ...
        for rep in range(reps):
            ...
            """
			Note that we dont want gradients passing through E loc
			"""
            E_loc_real_part, E_loc_imag_part = local_energy_function(samples, self)
        ...
        with tf.GradientTape() as tape:
            ...
            for sample_idx, samples in enumerate(all_samples):
				...
				(1/2)*D_log_p*Re[E_{loc} - E]+D_phase*Im[E_{loc} - E]
                ...
                if z_symmetric:
                    ...
                    loss += z_symmetric_reg * tf.reduce_sum(
                        (total_flip_log_probs - log_probs) ** 2
                    )
        ...
        if return_energy:
            return derivatives, (
                np.mean(E_estimate_real_parts),
                np.mean(E_estimate_imag_parts),
            )

        return derivatives

    def compute_nth_renyi(
        self, n: int, batch_size: int, region: Tuple[int, int], reps: int = 1
    ) -> float:
        """
        ...
        for _ in range(reps):
            ...
            (
                ...
                log_probabilities,
                autoregressive_phases,
            ) = self.autoregressive_sampling(seed_state)

            # reshape these to have the correct shape

            states = np.reshape(
                states, newshape=(batch_size, n, self.sequence_length - 1)
            )
            ...
            permuted_log_probabilities, permuted_total_phases = self.evaluate_state(
                np.reshape(
                    permuted_states, newshape=(batch_size * n, self.sequence_length - 1)
                )
            )
    ...
    def compute_von_neumann_estimate(
        ...
        coefficients = [np.log(2) - 1 / 2, np.log(2) - 3 / 4]
        coefficients += [
            (-1) ** (k + 1) / (k * (k**2 - 1)) for k in range(2, order + 1)
        ]
        ...
        for n in range(2, order + 1):
            ...
            nth_renyi = self.compute_nth_renyi(
                n=n, batch_size=batch_size, region=region, reps=reps
            )
